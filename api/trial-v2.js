...
export default async function handler(req, res) {
  ...
  const { session_id, trial_number, pair_id, position, left_product_id, right_product_id, left_variant, right_variant, rating, response_time_ms, is_catch_trial } = req.body;
  ...
  try {
    // Store left/right product IDs (and the variant shown on each side) in the
    // data JSONB column. Variant fields are optional and only added when present,
    // so a request without them produces a row identical to /api/trial's.
    const data = left_product_id && right_product_id
      ? { left_product_id, right_product_id }
      : null;
    if (data) {
      if (left_variant != null) data.left_variant = left_variant;
      if (right_variant != null) data.right_variant = right_variant;
    }
