import CONFIG from './config-movie-franchise.js';

// Adjust zoom for smaller viewports (laptops)
function adjustZoomForViewport() {
  // This study shows text (not fixed-height images), so the image-era
  // zoom-shrink is dropped — it needlessly hurt legibility of the primary
  // stimulus. Text reflows and the page scrolls if a short viewport needs it.
...
// other studies and this movie-franchise study don't share a completion flag.
...
// Check if user has already completed on server. Scoped to THIS experiment —
// participants who completed one of the other studies are not blocked here.
async function hasAlreadyCompletedOnServer() {
  if (!prolificPid) return false;

  try {
    const response = await fetch(`/api/session?prolific_pid=${encodeURIComponent(prolificPid)}&experiment_name=${encodeURIComponent(CONFIG.EXPERIMENT_NAME)}`);
...
function buildFamiliarityItems() {
  ...
  familiarityOrder.forEach(product => {
    ...
    // Name only — the franchise name is the stimulus in this study
...
async function init() {
  ...
  // Load products — each experiment keeps its stimuli (JSON) in its own
  // folder: stimuli/<EXPERIMENT_NAME>/
  try {
...
function setupEventListeners() {
  ...
  categoryContinueBtn.addEventListener('click', () => {
    const selected = document.querySelector('input[name="movie-days"]:checked');
    if (!selected) return;
...
function showTrial() {
  ...
  // Text-only display: the franchise name is the sole stimulus.
